...
int process_multithread(struct list* items, int nb_thread) {
  ...
  //Créer un "thread pool" avec nb_thread fils d'exécution
  struct pool* threadpool = threadpool_create(nb_thread);
  if (threadpool == NULL) {
    perror("Thread pool Creation Error");
    return 1;
  }

  // Ajouter la première image au thread pool
  threadpool_add_task(threadpool,process_one_image, list_head(items)->data);

  // Ajouter les images restantes au thread pool
  struct list_node* node = list_head(items)->next;
  while (node != items->sentinel) {
    threadpool_add_task(threadpool, process_one_image, node->data);
    node = node->next;
  }
